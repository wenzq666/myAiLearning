...
from src.top_news.config import Config  # 配置文件类
from src.top_news.training.model2dev_utils import model2dev  # 导入自定义验证函数(例如: 精确率, 召回率...)
from src.top_news.training.h1_dataloader_utils import build_dataloader  # 获取数据集加载器
from src.top_news.models.h2_bert_classifier_model import BertClassifier  # 导入BERT分类模型
...
def model2train():
    ...
    for epoch in range(conf.num_epochs):
        ...
        for batch in tqdm(train_dataloader):
            # 6.3.1 提取批次数据的input_ids, attention_mask, labels并移动到指定设备
            ...
            if batch_count % 100 == 0 or batch_count == len(train_dataloader):
                ...
                # 验证集的表现
                report, accuracy, precision, recall, current_f1 = model2dev(model=model, data_loader= dev_dataloader, device=conf.device)
                print(f"第{epoch + 1}轮,第{batch_count}次--准确率:{accuracy},精确率{precision},召回率{precision},f1值{current_f1}")
