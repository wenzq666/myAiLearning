...
from src.top_news.predict.rf_predict_fun import predict_fun as rf_fun
from src.top_news.predict.ft_predict_fun import predict_fun as ft_fun
from src.top_news.predict.h4_bert_predict_fun import predict_fun as bert_fun
from src.top_news.predict.h6_bilstm_predict_fun import predict_fun as bilstm_fun
...
@app.route("/ft_predict", methods=['POST'])
def ft_predict():
    # 获取用户请求中的数据,
    # "request" 是 Flask 提供的全局对象，包含了客户端发来的所有信息。
    # get_json() 会把接收到的 JSON 数据自动转换成 Python 字典。
...
def distill_predict():
    ...
    print(request_data)
    # 调用预测函数
    response = bilstm_fun(request_data)
    print(response)
